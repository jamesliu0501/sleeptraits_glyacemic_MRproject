...
# Therefore, we manually corrected the effect allele frequency of all the merged palindromic SNPs according to the data (i.e., minor allele frequency) downloaded from the MAGIC 
# The information of minor allele were obtained from the ensembl of EUR population (https://www.ensembl.org/index.html)
...
# Update the outcome into SD unit (for HbA1c, 1SD = 0.5342848% as shown in the MRbase, version: "MRCIEU/TwoSampleMR@0.4.26")
...
# To enable the comparison of the 2SMR estimates to the MVR and 1SMR results, 
# we converted the results of the SNP-binary sleep traits (i.e., insomnia, short sleep, and long sleep) from the multiplicative log odds scale to a difference in risk scale 
# by β=log⁡OR* μ*(1-μ)  ,se=selog⁡OR * μ*(1-μ), with μ=  n_case⁄((n_case+ n_control)
