...
# Model 1: adjusted for sex, age, assessment centre, 40 genetic principal components, smoking, alcohol intake, Townsend deprivation, education, sleep apnoea, and physical activity

mvr_a_log<-glm(log_hba1c~ sleepduration + 
                 sex + as.factor(assessment_centre) +
                 rc_numeric +  # age and PC1-40 are included here
                 townsend + 
                 as.factor(smoking) + as.factor(aif) + as.factor(education) + prevalent_apnoea + as.factor(vig),
               data = data)
